...
class BarcodeInput extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.auftrag_data) {
      console.log(nextProps.auftrag_data)
      if (tempComingFrom.toLowerCase() === 'home') {
        if (nextProps.auftrag_data.data.AVV_Nr !== null) {
          if (nextProps.auftrag_data.data.Prozesscode === '1' || nextProps.auftrag_data.data.Prozesscode === '1A') {
            this.props.history.push('TruckTypeHome', {
              state: {
    ...
    if (nextProps.first_weight_data && nextProps.first_weight_data.data.length > 0) {
      ...
      this.props.history.push('SignatureLable', {
        first_weight_data : nextProps.first_weight_data.data,
        comingFrom: tempComingFrom
      });
    } else {
      if (tempComingFrom.toLowerCase() !== 'home' && !nextProps.isFirstWeightDataLoading)
        this.setState({
          alert: {
            type: 'error',
            text: 'Yard ticket is not valid',
  ...
  gotoNext() {
    if (this.state.barcodeInputValue === '') {
      ...
      var tempComingFrom = this.props.location.state.comingFrom;
      localStorage.setItem('barcodeValue', this.state.barcodeInputValue);
      switch (tempComingFrom.toLowerCase()) {
        case 'annahmeabfallacceptancehome':
