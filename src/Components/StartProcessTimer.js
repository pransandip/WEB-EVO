...
class StartProcessTimer extends Component {
    ...
    render() {
        console.log('pdf data',this.state.pdfData);
        const processValue = this.props.location.state.process.variety_denomination;
        return (
            <div style={{
                overflow: 'hidden',
                ...
                <Container style={{ background: '', height: '435px' }} >
                    <Row style={{ marginTop: '40px' }}>
                        <div style={{ marginTop: '80px', marginLeft: 'auto', marginRight: 'auto', width: '603px', textAlign: 'center' }}>
                            <label style={{ fontSize: '25px', fontWeight: 'bold', color: '#000947', marginLeft: '20px', marginTop: '14px' }} >{localLangData[localStorage.getItem('lang')].waitToDrivelbl_general + " " + processValue}</label>
...
export default StartProcessTimer;
